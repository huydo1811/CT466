...
const fetchCategories = async () => {
  ...
  try {
    ...
    // controller returns { success,true, data: result.categories, pagination }
    const data = res?.data || {}
    items.value = data.data || data.categories || []
    pagination.value = data.pagination || data.pagination || { totalItems: items.value.length, totalPages: 1 }
  } catch (err) {
...
const openEdit = (c) => {
  editing.value = true
  form.value = { id: c._id || c.id, name: c.name, slug: c.slug, description: c.description }
  showModal.value = true
}
...
const onNameInput = () => { if (!editing.value) form.value.slug = slugify(form.value.name) }
...
const save = async () => {
  ...
  try {
    if (editing.value && form.value.id) {
      await api.put(`/categories/${form.value.id}`, { name: form.value.name, slug: form.value.slug, description: form.value.description })
    } else {
      await api.post('/categories', { name: form.value.name, slug: form.value.slug, description: form.value.description })
    }
    closeModal()
    fetchCategories()
  } catch (err) {
